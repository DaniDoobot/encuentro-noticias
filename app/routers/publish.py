...
@router.get("/publish/inspect-wp-fields")
def get_inspect_wp_fields():
    """
    Inspects the WordPress REST API for a real post to discover available meta/acf fields.
    Returns the schema properties and a real post's meta+acf fields (context=edit).
    Useful for debugging which custom fields are registered with show_in_rest=true.
    """
    ...
    url = (config.get("WORDPRESS_BASE_URL") or settings.WORDPRESS_BASE_URL or "").rstrip("/")
    username = config.get("WORDPRESS_USERNAME") or settings.WORDPRESS_USERNAME or ""
    app_password = settings.WORDPRESS_APPLICATION_PASSWORD or ""
    ...
    if not url or not username or not app_password:
        return {"error": "Missing WordPress credentials (WORDPRESS_BASE_URL, WORDPRESS_USERNAME, WORDPRESS_APPLICATION_PASSWORD)"}

    auth = _httpx.BasicAuth(username, app_password)
    results = {}

    with _httpx.Client(timeout=15.0) as client:
        # 1. Schema: what properties does the post type expose?
        try:
            schema_res = client.get(f"{url}/wp-json/wp/v2/{post_type}?context=edit&per_page=1", auth=auth)
            if schema_res.status_code == 200:
                posts = schema_res.json()
                if posts:
                    post = posts[0]
                    results["sample_post_id"] = post.get("id")
                    results["meta_keys"] = list(post.get("meta", {}).keys()) if isinstance(post.get("meta"), dict) else post.get("meta")
                    results["acf_keys"] = list(post.get("acf", {}).keys()) if isinstance(post.get("acf"), dict) else post.get("acf")
                    results["top_level_keys"] = list(post.keys())
                else:
                    results["note"] = "No posts found"
            else:
                results["posts_list_error"] = f"HTTP {schema_res.status_code}: {schema_res.text[:500]}"
        except Exception as e:
            results["posts_list_exception"] = str(e)

        # 2. WP REST API schema endpoint
        try:
            schema_res2 = client.get(f"{url}/wp-json/wp/v2/{post_type}?context=edit", auth=auth,
                                     headers={"Accept": "application/json"})
            # Also try the OPTIONS/schema
            opts_res = client.request("OPTIONS", f"{url}/wp-json/wp/v2/{post_type}", auth=auth)
            if opts_res.status_code == 200:
                schema_data = opts_res.json()
                schema_props = schema_data.get("schema", {}).get("properties", {})
                results["schema_properties"] = list(schema_props.keys())
                if "meta" in schema_props:
                    results["meta_schema"] = list(schema_props["meta"].get("properties", {}).keys())
                if "acf" in schema_props:
                    results["acf_schema"] = "present (ACF plugin active)"
        except Exception as e:
            results["schema_exception"] = str(e)

        # 3. Check ACF REST endpoint
        try:
            acf_res = client.get(f"{url}/wp-json/acf/v3/{post_type}", auth=auth)
            results["acf_endpoint_status"] = acf_res.status_code
            if acf_res.status_code == 200:
                acf_data = acf_res.json()
                if acf_data:
                    first = acf_data[0] if isinstance(acf_data, list) else acf_data
                    results["acf_fields_sample"] = list(first.get("acf", {}).keys()) if isinstance(first.get("acf"), dict) else str(first)[:300]
        except Exception as e:
            results["acf_endpoint_exception"] = str(e)
